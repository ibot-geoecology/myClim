...
test_that("mc_read_data csv without localities", {
    ...
    expect_equal(data$localities$A6W79$metadata@tz_type, mc_const_TZ_UTC)
    ...
    expect_equal(data$localities$A6W79$loggers[[1]]$metadata@type, myClim:::.model_const_LOGGER_TOMST_TMS_L45)
    ...
    expect_equal(data$localities$A2E32$loggers[[1]]$metadata@type, myClim:::.model_const_LOGGER_TOMST_TMS)
...
test_that("mc_read_files TOMST directory", {
    ...
    expect_equal(data$localities$`92192250`$loggers[[1]]$metadata@type, myClim:::.model_const_LOGGER_TOMST_DENDROMETER)
...
test_that("mc_read_data HOBO", {
    ...
    expect_true(myClim:::.model_const_SENSOR_HOBO_T_F %in% names(data$localities$C$loggers[[1]]$sensors))
...
test_that("mc_read_files TOMST with error in data", {
    ...
    states <- dplyr::filter(data$localities$data_93142777$loggers[[1]]$sensors$TMS_T2$states, tag == myClim:::.model_const_SENSOR_STATE_ERROR)
...
test_that("mc_read_wide", {
    ...
    expect_error(data <- mc_read_wide(data_table, myClim:::.model_const_SENSOR_precipitation, clean=FALSE))
    dates <- data_table$date
    data_table$date <- as.POSIXct(lubridate::ymd(dates))
    expect_error(data <- mc_read_wide(data_table, myClim:::.model_const_SENSOR_precipitation, clean=FALSE))
    data_table$date <- as.POSIXct(lubridate::ymd(dates, tz="UTC"))
    data <- mc_read_wide(data_table, myClim:::.model_const_SENSOR_precipitation, clean=FALSE)
    test_raw_data_format(data)
    expect_equal(length(data$localities), 49)
    expect_equal(names(data$localities$B1BLAT01$loggers[[1]]$sensors), myClim:::.model_const_SENSOR_precipitation)
