...
test_that("mc_calc_vpd", {
    ...
    raw_data <- mc_calc_vpd(cleaned_data, myClim:::.model_const_SENSOR_HOBO_T_C, myClim:::.model_const_SENSOR_HOBO_RH)
    test_raw_data_format(raw_data)
    expect_true("VPD" %in% names(raw_data$localities$LOC$loggers[[1]]$sensors))
    agg_data <- mc_agg(cleaned_data)
    agg_data <- mc_calc_vpd(agg_data, myClim:::.model_const_SENSOR_HOBO_T_C, myClim:::.model_const_SENSOR_HOBO_RH)
    test_agg_data_format(agg_data)
