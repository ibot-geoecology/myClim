...
`dataformat_name`. myClim can also read records from any wide
...
* **Handling time zones:** `mc_prep_solar_tz()` myClim expect input data in UTC time. It is ecologically meaningful to use rather solar time, which respects local photo-period especially when working with global data sets. Solar time is calculated from longitude of locality. The offset can be set manually, eg. to respect political time with `mc_prep_meta_locality()`.
...
# aggregate with your custom function. (how many records below -5°C per month)
