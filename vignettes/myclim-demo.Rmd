...
myClim stores data in lists and S3 classes. Records are  organised in localities, loggers and sensors. For detailed description and schema of myClim objects structure see `help("myClim")`  

## Reading microclimatic data
myClim supports natively import of several file formats: `r names(myClim::mc_data_formats)`. set parameter `dataformat_name`. myClim can also read records from any wide or long data frame in R. 
...
2.locality metadata e.g., coordinates, elevation, time zone offset...
...
Fast track to ecological analysis. Return single value for given period across all localities.    
```{r}
