...
tms.m <- mc_read_data(files_table = "d:/Git/myClim/examples/data/TOMST/files_table.csv",
...
After reading logger files or data frame into myClim object, such object is in Prep-format, format dedicated for preparations for analysis. Cleaning time series, handling time zones, cropping time, measured data calibration, conversion, adding metadata or joining multiple downloads of single logger in time (not implemented yet). 

```{r}
tms.clean<-mc_prep_clean(tms.d)
meteo.clean<-mc_prep_clean(meteo)
mc_info_count(tms.clean)
