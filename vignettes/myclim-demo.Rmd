...
```{r eval=TRUE}
# setwd("d:/Git/microclim/vignettes/")
...
hob.f <- mc_read_files(c("20024354.txt","20024354_comma.csv","20024354_semicolon.txt",
                         "20024354_tab.txt"), 
                       dataformat_name="HOBO",
                       date_format = "%d.%m.%y %H:%M:%S")
# 20024354.txt file is skipped, because of different date format. 

hob.f1 <- mc_read_files(c("20024354.txt"), 
                       dataformat_name="HOBO",
                       date_format = "%d.%m.%Y %H:%M:%S")
# 20024354.txt file correct date format definition "y=09" vs "Y=2009" 
...
tms1<-tms
names(tms1)
tms1<-mc_prep_meta_locality(tms1, list(A1E05="ABC05", A2E32="CDE32"), param_name="locality_id")
tms1<-mc_prep_meta_sensor(tms1, values=list(TMS_T1="TMS_Tsoil", TMS_T2="TMS_Tair"),
                          localities = "A6W79", param_name="name")
...
tms.f <- mc_read_files(c("data_91184101_0.csv","data_94184102_0.csv","data_94184103_0.csv"),
                       dataformat_name="TOMST")

## construct metadata data.frame
meta<-read.table("localities_table.csv",sep=",", header = T)
paths<-read.table("files_table.csv",sep=",", header = T)
mer<-merge(paths,meta,by="locality_id")
mer$id_auto<-substr(mer$path,nchar(mer$path)-13,nchar(mer$path)-6)
...
## upload metadata from list
lst<-as.list(mer$tz_offset)
names(lst)<-mer$id_auto
tms.f<-mc_prep_meta_locality(tms.f, lst, param_name = "tz_offset")
...
Output table of `mc_info()`

```{r, results = "asis",echo=FALSE,error=FALSE,warning=FALSE}
library(kableExtra)

kable(head(mc_info(tms.day),10), "html", digits=2) %>%
