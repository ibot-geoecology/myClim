...
#'
#' @details
#' In default settings it returns the object containing input localities / loggers / logger types / sensors.
...
#'
#' Parameter `reverse = TRUE` returns myClim object without listed localities or loggers or logger types or sensors.
#' Using  `reverse = TRUE` is not allowed for combination of localities and loggers and logger types and sensors.
#' It is allowed to use `reverse` only with single filter criterion either locality, logger, logger type or sensor.
#' 
...
mc_filter <- function(data, localities=NULL, logger_types=NULL, loggers=NULL,
    ...
    if(reverse && sum(c(!is.null(localities), !is.null(sensors), !is.null(logger_types), !is.null(loggers))) > 1) {
    ...
    sensors_item_function <- function(item) {
        ...
        if(is.null(logger_types) && is.null(loggers)) {
            is_in <- TRUE
        } else if(!is.null(loggers)) {
            is_in <- item$metadata@name %in% loggers
        } else if(!is.null(logger_types)) {
            is_in <- item$metadata@type %in% logger_types
        }
    ...
    locality_function <- function(locality) {
        ...
        if(is.null(sensors) && is.null(logger_types) && is.null(loggers)) {
    ...
    if(is.null(localities) && is.null(sensors) && is.null(logger_types) && is.null(loggers) && !reverse) {
        return(data)
    }
    if(is.null(localities) && is.null(sensors) && is.null(logger_types) && is.null(loggers) && reverse) {
