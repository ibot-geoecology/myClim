...
states_table <- mc_info_states(mc_data_example_raw)
paths <- as.data.frame(stringr::str_match(states_table$value, ".+(\\/myClim\\/examples\\/data\\/.+)"))$V2
states_table$value <- paste0(".", paths)
mc_data_example_raw <- .states_run(mc_data_example_raw, states_table, .states_update, FALSE)
